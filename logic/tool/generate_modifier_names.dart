...
String generateDartFile(Set<String> names) {
  ...
  buffer.writeln('// modifier names found in the Melvor Idle JSON data files.');
  buffer.writeln();
  buffer.writeln('import \'package:meta/meta.dart\';');
  buffer.writeln();

  // ModifiersBase class with getters
  buffer.writeln('/// Base class providing typed access to modifier values.');
  buffer.writeln('///');
  buffer.writeln(
    '/// Provides getters for all known modifier names. Values default to 0',
  );
  buffer.writeln('/// when not present in the underlying map.');
  buffer.writeln('@immutable');
  buffer.writeln('class ModifiersBase {');
  buffer.writeln('  const ModifiersBase(this.values);');
  buffer.writeln();
  buffer.writeln('  /// The underlying modifier values map.');
  buffer.writeln('  final Map<String, num> values;');
  buffer.writeln();
  buffer.writeln('  /// True if no modifiers are present.');
  buffer.writeln('  bool get isEmpty => values.isEmpty;');
  buffer.writeln();
