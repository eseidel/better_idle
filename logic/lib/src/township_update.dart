...
/// Modifier values relevant to township processing.
///
/// Extracted from the full modifier system so township update logic
/// doesn't depend on the full ModifierProvider.
@immutable
class TownshipModifiers {
  const TownshipModifiers({
    this.gpProduction = 0,
    this.maxStorage = 0,
    this.repairCost = 0,
    this.resourceProduction = const {},
    this.taxPerCitizen = 0,
    this.traderCost = 0,
    this.flatEducation = 0,
    this.flatHappiness = 0,
  });

  /// Percentage modifier to GP generated by township (e.g., 10 = +10%).
  final int gpProduction;

  /// Percentage modifier to maximum storage capacity (e.g., 20 = +20%).
  final int maxStorage;

  /// Percentage modifier to building repair costs (e.g., -10 = 10% cheaper).
  final double repairCost;

  /// Per-resource percentage modifier to resource production.
  /// Keys are resource MelvorIds (actionId scope in modifier system).
  final Map<MelvorId, int> resourceProduction;

  /// Percentage modifier to tax per citizen (e.g., 5 = +5%).
  final int taxPerCitizen;

  /// Percentage modifier to trader costs (e.g., -10 = 10% cheaper).
  final double traderCost;

  /// Flat bonus to education stat (added after building contributions).
  final int flatEducation;

  /// Flat bonus to happiness stat (added after building contributions).
  final int flatHappiness;

  /// Returns the resource production modifier for a specific resource.
  int resourceProductionFor(MelvorId resourceId) =>
      resourceProduction[resourceId] ?? 0;
}

/// Default modifiers with no effects (all zeros).
const defaultTownshipModifiers = TownshipModifiers();
...
class TownshipStats {
  ...
  ///
  /// [modifiers] applies flat happiness/education bonuses and storage
  /// capacity modifiers from equipment, potions, pets, etc.
  factory TownshipStats.calculate(
    TownshipState state,
    TownshipRegistry registry, {
    TownshipModifiers modifiers = defaultTownshipModifiers,
  }) {
    ...
    // Apply flat modifier bonuses (from equipment, potions, pets, etc.)
    happiness += modifiers.flatHappiness;
    education += modifiers.flatEducation;

    // Apply townshipMaxStorage modifier (percentage)
    if (modifiers.maxStorage != 0) {
      storage = (storage * (1.0 + modifiers.maxStorage / 100.0)).floor();
    }
  ...
  for (final biomeEntry in state.biomes.entries) {
    ...
    for (final buildingEntry in biomeState.buildings.entries) {
      ...
      for (final prodEntry in biomeData.production.entries) {
        ...
        // Per-resource production modifier (percentage)
        final resourceMod = modifiers.resourceProductionFor(resourceId);
        final resourceMultiplier = 1.0 + (resourceMod / 100.0);
        ...
        // - Per-resource modifier (townshipResourceProduction)
        final amount =
            (baseAmount *
                    count *
...
/// [modifiers] provides equipment/potion/pet bonuses to township stats.
TownshipUpdateResult processTownUpdate(
  ...
  final stats = TownshipStats.calculate(state, registry, modifiers: modifiers);
  ...
  final production = _calculateProduction(
    state,
    registry,
    stats,
    modifiers: modifiers,
  ...
  // Calculate tax GP from citizens (townshipTaxPerCitizen modifier)
  if (modifiers.taxPerCitizen != 0) {
    final taxGp = (stats.effectivePopulation * modifiers.taxPerCitizen / 100.0)
        .floor();
    gpProduced += taxGp;
  }
  ...
  // Apply townshipGPProduction modifier to total GP
  if (modifiers.gpProduction != 0 && gpProduced > 0) {
    gpProduced = (gpProduced * (1.0 + modifiers.gpProduction / 100.0)).floor();
  }
